...
VALID_NAME = "Телефон"
VALID_PRICE = 90850
VALID_STATISTICS = {"likes": 10, "viewCount": 10, "contacts": 10}
...
def get_item_by_id(item_id):
    response = requests.get(
        f"{BASE_URL}/api/1/item/{item_id}", headers={"Accept": "application/json"}
    )
    return response
...
def get_items_by_seller(seller_id):
    response = requests.get(
        f"{BASE_URL}/api/1/{seller_id}/item", headers={"Accept": "application/json"}
    )
    return response
...
def get_statistics(item_id):
    response = requests.get(
        f"{BASE_URL}/api/2/statistic/{item_id}", headers={"Accept": "application/json"}
    )
    return response
...
def delete_item(item_id):
    response = requests.delete(
        f"{BASE_URL}/api/2/item/{item_id}", headers={"Accept": "application/json"}
    )
    return response
...
@allure.description(
    "Проверяет успешное создание объявления с валидными данными (статистика = 0)"
)
...
def test_create_item_success():
    ...
    with allure.step("Проверка наличия поля status в ответе"):
        ...
        ), f"Ответ не содержит поле status: {response_json}"
...
@allure.description(
    "Проверяет, что API возвращает ошибку 400 при отсутствии поля sellerID"
)
@allure.severity(allure.severity_level.CRITICAL)
def test_create_item_missing_seller_id():
    payload = {"name": VALID_NAME, "price": VALID_PRICE, "statistics": VALID_STATISTICS}
...
def test_get_item_multiple_times():
    ...
    with allure.step("Создание тестового объявления"):
        ...
        ), f"Ожидался 200, получили {create_response.status_code}"
    ...
    with allure.step("Проверка статус-кодов 200"):
        ...
        ), f"Ожидался 200, получили {response1.status_code}"
        assert (
            response2.status_code == 200
        ), f"Ожидался 200, получили {response2.status_code}"
        assert (
            response3.status_code == 200
        ), f"Ожидался 200, получили {response3.status_code}"
    with allure.step("Проверка идентичности ответов"):
        assert (
            response1.json() == response2.json() == response3.json()
        ), f"Ответы запросов не идентичны: {response1.json()} {response2.json()} {response3.json()}"


# API-004 Полный жизненный цикл объявления (CRUD операция)
@allure.title("API-004: Полный жизненный цикл объявления (CRUD)")
@allure.description(
    "Проверяет полный цикл: создание, чтение, чтение по продавцу, чтение статистики и удаление"
)
@allure.severity(allure.severity_level.CRITICAL)
def test_full_lifecycle():
    ...
    with allure.step("Получение объявления по ID"):
        ...
        ), f"Ожидался 200, получили {get_response.status_code}"
    ...
    with allure.step("Получение объявлений продавца"):
        ...
        assert (
            seller_items_response.status_code == 200
        ), f"Ожидался 200, получили {seller_items_response.status_code}"
        item_ids = []
        for item in seller_items_response.json():
            item_ids.append(item["id"])
        assert (
            item_id in item_ids
        ), f"Среди объявлений продавца не найдено созданного ранее объявления: {item_ids}"
    ...
    with allure.step("Получение статистики"):
        ...
        ), f"Ожидался 200, получили {stats_response.status_code}"
        stats = stats_response.json()[0]
        assert (
            stats["likes"] == VALID_STATISTICS["likes"]
        ), f"В полученной статистике не найдено поле likes: {stats}"
        assert (
            stats["viewCount"] == VALID_STATISTICS["viewCount"]
        ), f"В полученной статистике не найдено поле viewCount: {stats}"
        assert (
            stats["contacts"] == VALID_STATISTICS["contacts"]
        ), f"В полученной статистике не найдено поле contacts: {stats}"
...
def test_xss_protection():
    ...
    with allure.step("Проверка экранирования опасных символов"):
        ...
        assert (
            "\\u003cscript\\u003e" in raw_text
        ), f"В тексте ответе нет экранирования: {raw_text}"

        # Или проверяем, что нет незаэкранированных символов
        assert (
            "<script>" not in raw_text
        ), f"В тексте ответа есть незаэкранированный тег: {raw_text}"
